<template>

    <header class="header">
        <div class="content-wrapper">
            <div class="header-img">
                <img width="64px" height="64px" src="http://static.galileo.xiaojukeji.com/static/tms/seller_avatar_256px.jpg" alt="">
            </div>
            <div class="content">
                <div class="title">   
                    <span class="brands"></span>
                    <span class="name">粥品香坊（石坝村）</span>
                </div>
                <div class="description">
                蜂鸟专送 / 38分钟送达
                </div>
                <div class="supports">
                    <span class="icon decrease"></span>
                    <span class="text">在线支付满28减10</span>
                </div>
                <div class="support-count">
                    <span class="count">5个</span>
                    <i class="fa fa-angle-right"></i> 
                </div>
            </div>
        </div>
        <div class="bulletin-wrapper">
            <span class="notice"></span>
            <span class="text">
                粥品香坊其烹饪粥料的秘方源于中国千年古法，在融和现代制作工艺，由世界烹饪大师屈浩先生领衔研发。坚守纯天然、0添加的良心品质深得消费者青睐，发展至今成为粥类的引领品牌。是2008年奥运会和2013年园博会指定餐饮服务商。
            </span>
            <i class="fa fa-angle-right"></i> 
        </div>
        <div class="background">
            <img width="100%;" height="100%;" src="http://static.galileo.xiaojukeji.com/static/tms/seller_avatar_256px.jpg" alt="">
        </div>
    </header>
</template>

<style>
    .header{
        display: block;
        background: rgba(7,17,27,0.5);
        overflow: hidden;
        color: #ffffff;
    }
    .content-wrapper{
        position: relative;
        display: flex;
        padding: 24px 12px 18px 24px;
        font-size: 12px;
    }
    .content-wrapper .content{
        margin-left: 16px;
    }
    .content-wrapper .title{
        margin: 2px 0 8px 0;
        font-size: 16px;
    }
    .content-wrapper .title .brands{
        display: inline-block;
        width: 38px;
        height: 18px;
        vertical-align: top;
        background-image: url(data:image/png;base64,);
        background-size: 30px 18px;
        background-repeat: no-repeat;
    }
    .content-wrapper .title .name{
        font-weight: bold;
        font-size: 16px;
        line-height: 18px;
    }
    .header .description{
        font-size: 12px;
        margin-bottom: 10px;
        padding-left: 3px;
    }
     .supports .icon{
        display: inline-block;
        vertical-align: top;
        width: 12px;
        height: 12px;
        margin-right: 4px;
        background-size: 12px 12px;
        background-repeat: no-repeat;
    }
    .supports .icon.decrease{
        background-image: url(data:image/png;base64,);

    }
     .supports .text{
         vertical-align: top;
         line-height: 14px;
     }
     .support-count{
        position: absolute;
        right: 12px;
        bottom: 18px;
        padding: 0 14px;
        height: 24px;
        line-height: 24px;
        border-radius: 14px;
        background-color: rgba(0,0,0,0.2);
        text-align: center;
     }
    .support-count i{
        margin-left: 2px;
    }
    .background{
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
        height: 108px;
        filter: blur(10px);
        z-index: -1;
    }
    .bulletin-wrapper{
        position: relative;
        height: 28px;
        line-height: 28px;
        padding: 0 22px 0 12px;
        white-space: nowrap;
        overflow: hidden;
        text-overflow: ellipsis;
        font-size: 10px;
        background: rgba(0,0,0,0.2);
    }
    .bulletin-wrapper .notice{
        background-image: url(data:image/png;base64,);
        position: relative;
        width: 22px;
        height: 12px;
        display: inline-block;
        vertical-align: top;
        margin-top: 9px;
        background-size: 100% 100%;
        background-repeat: no-repeat;
    }
    .bulletin-wrapper .text{
        margin-left: 8px;
        vertical-align: middle;
    }
    .bulletin-wrapper i{
        position: absolute;
        font-size: 10px;
        right: 12px;
        top: 8px;
    }
</style>

<script>
export default {
    
}

</script>